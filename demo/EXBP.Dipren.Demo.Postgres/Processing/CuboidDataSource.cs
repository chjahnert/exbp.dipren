    ...
    internal class CuboidDataSource : IDataSource<Guid, Cuboid>
    {
        ...
        public async Task<Range<Guid>> GetEntireRangeAsync(CancellationToken cancellation)
        {
            ...
            using (NpgsqlConnection connection = new NpgsqlConnection(this._connectionString))
            {
                ...
                using NpgsqlCommand command = new NpgsqlCommand
                {
                    CommandText = "SELECT (SELECT \"id\" FROM \"demo\".\"cuboids\" ORDER BY \"id\" ASC LIMIT 1) AS \"first\", (SELECT \"id\" FROM \"demo\".\"cuboids\" ORDER BY \"id\" DESC LIMIT 1) AS \"last\";",
                    CommandType = CommandType.Text,
                    Connection = connection
                };
        ...
        public async Task<IEnumerable<KeyValuePair<Guid, Cuboid>>> GetNextBatchAsync(Range<Guid> range, int skip, int take, CancellationToken cancellation)
        {
            ...
            using (NpgsqlConnection connection = new NpgsqlConnection(this._connectionString))
            {
                ...
                using NpgsqlCommand command = new NpgsqlCommand
                {
                    CommandText = "SELECT \"id\", \"width\", \"height\", \"depth\" FROM \"demo\".\"cuboids\" WHERE (\"id\" >= @first) AND (((@inclusive = TRUE) AND (\"id\" <= @last)) OR ((@inclusive = FALSE) AND (\"id\" < @last))) ORDER BY \"id\" ASC OFFSET @skip LIMIT @take;",
                    CommandType = CommandType.Text,
                    Connection = connection
                };
